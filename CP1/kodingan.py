...
POSTGIS_TABLE = dict(
    ...
    password='ebud',
    dbname='tugasgisebud',
    table='(select ST_Buffer(ST_Centroid(geom),1) as geom, nama from coba) as coba',
    # table='(select ST_LineMerge(ST_AsText(geom)) as geom, nama from coba) as coba'
    # table='(select ST_MPolyFromText(ST_AsText(geom)) as geom, nama from jatim) as jatim'
    )
LAYER_NAME = 'pusat_kesehatan'
...
def render_tile(layer, z, x, y):
    ...
    line_symbolizer.fill = mapnik.Color('red')
    line_symbolizer.fill_opacity = 0.3
    #line_symbolizer.allow_overlap = True
    #line_symbolizer.transform="scale(5,5)"
    r.symbols.append(line_symbolizer)

    label = mapnik.TextSymbolizer(mapnik.Expression('[nama]'), 'DejaVu Sans Book',12,mapnik.Color('green'
    ))
    label.halo_radius = 1
